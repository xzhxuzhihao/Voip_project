...
//2023-7-11 add 发送invite消息，用于建立会话
int  ClientInvite(char *src, char *dst, char* sipstr )
{
    int i;
	char tmpcallId[64];
    char tmp[4096];
    ...
    i = eXosip_call_build_initial_invite (context_eXosip, &invite, dst, src, NULL, sipstr);
    if (i != 0)
    {
	...
	sprintf (tmp, 
			  "v=0\r\n"
			  "o=jyd-r&d 0 0 IN IP4 %s\r\n"
			  "s=Play\r\n"
			  "c=IN IP4 %s\r\n"
			  "t=0 0\r\n"
			  "m=video 9902 RTP/AVP 96 97 98\r\n"
			  "a=recvonly\r\n"
			  "a=rtpmap:96 PS/90000\r\n"
			  "a=rtpmap:97 MPEG4/90000\r\n"
			  "a=rtpmap:98 H264/90000\r\n", invite->call_id->host, invite->call_id->host);

	
    osip_message_set_body (invite, tmp, strlen(tmp));
    osip_message_set_content_type (invite, "application/sdp");
    osip_message_set_header (invite, "Allow","Invite,ACK,UPDATE,INFO,CANCEL,BYE,OPTIONS,REFER,SUBSCRIBE,NOTIFY,MESSAGE");
...
unsigned int __stdcall sip_main(void *) 
{
  ...
  for (; keepRunning;) {
    ...
    case EXOSIP_REGISTRATION_FAILURE:
      break;
			...
			//处理携带SDP的200OK消息，可能后续需要添加一些对媒体发送设备的一些信息的提取
        case EXOSIP_CALL_ANSWERED:
            ...
            eXosip_call_build_ack (context_eXosip, event->did, &ack);
			
			//vias

			eXosip_call_send_ack (context_eXosip, event->did, ack);
...
#define short_options "du:r:U:P:t:p:c:e:mf:h"
...
#else
    c = xgetopt(argc, argv, short_options);
#endif
    ...
    case 'p':
      service = getservbyname(xoptarg, "udp");
    ...
    case 't':
      snprintf(transport, sizeof(transport), "%s", xoptarg);
      break;
...

  {
   ...
   handle = (HANDLE)_beginthreadex(NULL, 0, sip_main, NULL, 0, NULL);

 // HANDLE hThread = CreateThread(NULL, 0, sip_main, NULL, 0, NULL);
  ...
  while (1)
    {
      ...
      getchar ();
		...
		{
			 case '1'://register
				 ClientInvite("sip:101@192.168.1.3:6000", "sip:102@192.168.1.3:6000" ,NULL); 
				break;    
			 case 'q':
				 return 0;
			 default:
