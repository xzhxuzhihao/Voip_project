...
//	  printf("data : in exosip, jr->r_id == %d\n",jr->r_id);
//	  printf("data : in exosip, ptr:jr == %p\n",jr);
//    if (jr->r_id == rid) {
//      return jr;
//    }
//  }
//  if(jr == NULL)
//  printf("in exosip,ptr: jr == %p\n",jr);
//
//  return NULL;
//}
//
...
//  printf("_eXosip_regis_bui 1  jr->r_last_tr = %d\n", jr->r_last_tr);
...
//
//  if (reg == NULL) {
//	  printf("_exosip _eXosip_generating_register. \n");
...
int eXosip_test_1(struct eXosip_t *excontext){
  if(excontext != NULL){
    printf("test:excontext_1 is %p\n", excontext);
    printf("test:excontext->j_reg_1 is %p\n", excontext->j_reg);
  }
  if(excontext->j_reg != NULL){
    printf("test:excontext->j_reg->r_id_1 is %d\n", excontext->j_reg->r_id);
  }
  return 0;

}
int eXosip_register_send_register_1(struct eXosip_t *excontext, int rid, osip_message_t *reg) {
  //osip_transaction_t *transaction; osip_event_t *sipevent;
  //eXosip_reg_t *jr; int i; if (rid <= 0) {
  //  osip_message_free(reg);
  //  return OSIP_BADPARAMETER;
  //}
  if(excontext != NULL){
    printf("excontext_1 is %p\n", excontext);
    printf("excontext->j_reg_1 is %p\n", excontext->j_reg);
  }
  if(excontext->j_reg != NULL){
    printf("excontext->j_reg->r_id is %d\n", excontext->j_reg->r_id);
  }
  //jr = eXosip_reg_find(excontext, rid);
