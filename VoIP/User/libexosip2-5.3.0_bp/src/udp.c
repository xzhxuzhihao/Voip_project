...
// recieve message from the underlying interface\
// max_message_nb is max_message_to_read
int _eXosip_read_message(struct eXosip_t *excontext, int max_message_nb, int sec_max, int usec_max) {
  fd_set osip_fdset;  // fd_set is a structure include an array
  fd_set osip_wrset;
  ...
  tv.tv_sec = sec_max;
  tv.tv_usec = usec_max;
// the j_stop_ua use to indicate some stop signal
  while (max_message_nb != 0 && excontext->j_stop_ua == 0) {
    int i;
    int max = 0;

#ifndef OSIP_MONOTHREAD
		// return the socket[0] which is inited in exosip_init()
    ...
    if (excontext->poll_method == EXOSIP_USE_SELECT) {
      int osip_fd_table[EXOSIP_MAX_SOCKETS];
      FD_ZERO(&osip_fdset); // set to zero
      FD_ZERO(&osip_wrset);
      FD_ZERO(&osip_exceptset);
      memset(osip_fd_table, -1, sizeof(osip_fd_table)); // set all integer to -1
			// set socket to osip_fdset, osip_wrset, osip_exceptset, osip_fd_table
			// just set the osip_fdset
      ...
      if ((sec_max == -1) || (usec_max == -1))
        ...
        i = select(max + 1, &osip_fdset, &osip_wrset, &osip_exceptset, &tv);
			//if there are errors then continue
...
#ifndef OSIP_MONOTHREAD
			// the select's return is success
      if ((i > 0) && FD_ISSET(wakeup_socket, &osip_fdset)) {
        char buf2[500];

        jpipe_read(excontext->j_socketctl, buf2, 499);
			// clear the osip_fdset
        FD_CLR(wakeup_socket, &osip_fdset);
      }

#endif
			// j_stop_ua != 0, indicate to stop
			// i == 0 indicate time out!
      ...
      if (-1 == i) {
        ...
        return -2000;    /* error */
				...
				// i > 0, some message should be read
        eXosip_lock(excontext);
        for (i = 0; osip_fd_table[i] != -1; i++) {
