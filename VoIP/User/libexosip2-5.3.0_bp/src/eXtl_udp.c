...
//use get_addrinfo() and socket(),bind() function to init the space pointed by excontext->eXtludp_reserved
// the new udp socket  is stored at reserved->udp_socket
static int _udp_tl_open(struct eXosip_t *excontext, int force_family) {
 ...
 // The proto_port is 5060 by default
...
// the proto_ifs is "0.0.0.0" by default
  ...
  if (res)
    return -1;
// find out all the host ip address information
...
static int _udp_tl_open_oc(struct eXosip_t *excontext, int force_family) {
  ...
  int res;
  struct addrinfo *addrinfo = NULL;
	// the struct addrinfo is a chain table type
  struct addrinfo *curinfo;
  int sock = -1;
  char eb[ERRBSIZ];
...
// alloc sturct eXtludp and then call _udp_tl_open()
...
static int _udp_read_udp_main_socket(struct eXosip_t *excontext) {
  ...
  char eb[ERRBSIZ];
	...
	//sockaddr is used for various ip_families
  if (reserved->udp_socket_family == AF_INET)
  ...
  if (reserved->buf == NULL)
    return OSIP_NOMEM;
	// use sa to save the source address of message
    ...
    if ((reserved->buf[0] == 0 || reserved->buf[0] == 1)) {
      ...
      if (ntohs(data->length) + 20 != i) {
        ...
        return OSIP_SUCCESS;
			...
			// find out the register information of this message
      for (jr = excontext->j_reg; jr != NULL; jr = jr->next) {
				// stun_binding.tr_id is the transaction's identifier of stun request
				// why not 64 bits offset in reserved->buf 
        if (memcmp(reserved->buf + sizeof(uint32_t), jr->stun_binding.tr_id, 12)) {
          ...
          struct osip_stun_atr *atr = (struct osip_stun_atr *) (reserved->buf + 20);
					// ntohs() convert the network-byte order to host-byte order
          ...
          if ((atr_typ == 0x0020 || atr_typ == 0x8020) && (atr_len == 8 || atr_len == 20)) {
            ...
            if (atr_len == 8) {
              ...
              naddr = ntohl(naddr);
							// decode the ip address which is coded by XOR
              _stun_decode_xor_address_ipv4(jr->stun_binding.tr_id, &nport, &naddr);
							//host to network formal
              naddr = htonl(naddr);
							// convert the binary ip address to human-readable ip address ('ipbuf')
...
static int udp_tl_read_message(struct eXosip_t *excontext, fd_set *osip_fdset, fd_set *osip_wrset, fd_set *osip_exceptset) {
  ...
  if (reserved->udp_socket < 0)
    return -1;
	//FD_SET(n,p) is assign n to the set p
	//FD_CLR(n,p) is used to clear n in set p
	//FD_ISSET is used to check whether the n is in set p 
	// oc is oubound connection
...
static struct eXtl_protocol eXtl_udp = {1,
                                        ...
                                        &udp_tl_init,  // allocate storage space
                                        &udp_tl_free,  //free the dynamic space
                                        &udp_tl_open, 
                                        &udp_tl_set_fdset,
                                        &udp_tl_read_message,
#ifdef HAVE_SYS_EPOLL_H
