...
struct eXtl_protocol {
  ...
  int proto_port;   // 5060
  char proto_name[10];
  char proto_ifs[64];  // set the interface to listen on some ip address, maybe this array is used to bind with socket
  int proto_num;
  int proto_family;
  int proto_secure; // set to 0 by default
  int proto_reliable; // set to 0
  int proto_local_port; // set to 0 
  ...
  int (*tl_read_message)(struct eXosip_t *excontext, fd_set *osip_fdset, fd_set *osip_wrset, fd_set *osip_exceptset);
