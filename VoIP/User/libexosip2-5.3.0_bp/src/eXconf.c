...
#endif
//get some server socket with the information came from user
//eXosip_listen_addr(context, IPPROTO_UDP, NULL, port, AF_INET, 0)
int eXosip_listen_addr(struct eXosip_t *excontext, int transport, const char *addr, int port, int family, int secure) {  // addr = NULL
  int i = -1;

  if (excontext->eXtl_transport.enabled > 0) {
    /* already set */
    OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, "[eXosip] already listening somewhere\n"));
		printf("1 %d\n", excontext->eXtl_transport.enabled);
    return OSIP_WRONG_STATE;
  }

  if (transport == IPPROTO_UDP && secure == 0)
		// init some udp parameters, which will be setted to eXtl_protocol
    eXosip_transport_udp_init(excontext);
    ...
    return OSIP_BADPARAMETER;
// alloc and init the eXtludp_reserved of excontext
  if (excontext->eXtl_transport.tl_init != NULL)
    excontext->eXtl_transport.tl_init(excontext);
...
#endif
	// open a udp server socket and save it in excontext->eXtludp_reserved
...
struct eXosip_t *eXosip_malloc(void) {
  struct eXosip_t *ptr = (struct eXosip_t *) osip_malloc(sizeof(eXosip_t)); //osip_malloc is malloc
...
//init the ipv4, ipv6, max_message_to_read, dscp, implicit_subscription_expires, 
//user_agent, j_calls, j_stop_ua, j_reg, j_transactions, sturct osip_t, application_context, 
//callback function of osip_t, j_osip, j_events, a series of variable from use_rport to use_ephemeral_port or poll_method,
//max_fd_no, epfd, ep_array, 
int eXosip_init(struct eXosip_t *excontext) {
  ...
  memset(excontext, 0, sizeof(eXosip_t));
	printf("2 %d\n", excontext->eXtl_transport.enabled);
	//do no things if defined the MINISIZE -XU
...
#endif
	// osip_strdup is used for malloc memery and copy to it.
	// user_agent is inited to "eXosip/5.3.0"
...
//if is MONOTHREAD mode
#ifndef OSIP_MONOTHREAD
  excontext->j_thread = NULL;
#endif
	//osip_list_init() inits the list using memset()
...
//init the osip structrue
  ...
  if (i != 0) {
	...
	// set the application_context pointer of osip_t to excontext 
	// This will make the osip_t and exosip_t to point together 
  osip_set_application_context(osip, &excontext);
//Set the callback function to osip related structure
  _eXosip_set_callbacks(osip);
// connect the osip_t to exosip_t
  excontext->j_osip = osip;

#ifndef OSIP_MONOTHREAD
  /* open a TCP socket to wake up the application when needed. */
	// set a pipe, and pipe[0] to write, pipe[1] to read
	// The jpipe() use the bind(), socket(), connect()... to build a connection of tcp
  excontext->j_socketctl = jpipe();

  if (excontext->j_socketctl == NULL)
    return OSIP_UNDEFINED_ERROR;
	//jpipe() return a pointer of jpipe_t
  ...
  /* To be changed in osip! */
	// The j_events have a list table
...
int eXosip_execute(struct eXosip_t *excontext) {
  ...
  if (excontext->max_read_timeout > 0) {
	...
	// get the minimum time of timer
    osip_timers_gettimeout(excontext->j_osip, &lower_tv);
		// more than 10s before time out
    if (lower_tv.tv_sec > 10) {
      eXosip_reg_t *jr;
      time_t now;
			// it's for android device
      osip_compensatetime();
			// use osip_gettimeofday to get current time; return the sec value of time
      now = osip_getsystemtime(NULL);

      lower_tv.tv_sec = 10;

      eXosip_lock(excontext);
			// the j_reg is a pointer of  registrationsn
			// the j_reg is NULL by default
			// iterate the j_reg chain table
  ...
  if (excontext->cbsipWakeLock != NULL && excontext->outgoing_wake_lock_state == 0) {
	...
	// execute some function periodically
...
static void _eXosip_keep_alive(struct eXosip_t *excontext) {
  ...
  osip_gettimeofday(&excontext->ka_timer, NULL);
  add_gettimeofday(&excontext->ka_timer, excontext->ka_interval);
	// excute the keepalive function periodically
