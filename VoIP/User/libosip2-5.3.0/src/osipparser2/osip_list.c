...
// get the fisrt node's element in chain table, and the iterator maintains the chain table which is the iterator of list structure
void *osip_list_get_first(const osip_list_t *li, osip_list_iterator_t *iterator) {
  ...
  iterator->pos = 0; // the position of node
