...
osip_transaction_t *__osip_find_transaction(osip_t *osip, osip_event_t *evt, int consume) {
  ...
  if (EVT_IS_INCOMINGMSG(evt)) {
    if (MSG_IS_REQUEST(evt->sip)) {
      if (0 == strcmp(evt->sip->cseq->method, "INVITE") || 0 == strcmp(evt->sip->cseq->method, "ACK")) {
...
// init the parser and transaction of sip
...
//if use threads then initting the mutex
#ifndef OSIP_MONOTHREAD
...
int osip_ict_execute(osip_t *osip) {
  ...
  for (index = 0; index < len; ++index) {
    ...
    do {
			// get event and remove the getting one
			// get the event
      ...
      if (se == NULL) /* no more event for this transaction */
		...
		// execute those events until the se == NULL 
  }
...
void osip_timers_gettimeout(osip_t *osip, struct timeval *lower_tv) {
  ...
  osip_mutex_lock(osip->ict_fastmutex);
#endif
  /* handle ict timers */ // ict means the invite client transaction
  tr = (osip_transaction_t *) osip_list_get_first(&osip->osip_ict_transactions, &iterator);

  while (osip_list_iterator_has_elem(iterator)) {
		// the osip_fifo_size return the number of nodes(events at here)
    if (1 <= osip_fifo_size(tr->transactionff)) {
      OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO4, NULL, "1 Pending event already in transaction !\n"));
			// recieve new event, so the timer setted to 0
      lower_tv->tv_sec = 0;
      lower_tv->tv_usec = 0;
#ifndef OSIP_MONOTHREAD
      osip_mutex_unlock(osip->ict_fastmutex);
      ...
      return;
			...
			// the min_timercmp() return the smaller value to lower_tv
      ...
      if (osip_timercmp(&now, lower_tv, >=)) {
        ...
        osip_mutex_unlock(osip->ict_fastmutex);
#endif
				// come back when find a time out
        return;
      }
    }
...
// iterator the element in chain list
...
#endif
  {
		//the ixt_t is a Structure for 2XX retransmission management.
    ixt_t *ixt;
  ...
  osip_mutex_unlock(osip->ixt_fastmutex);
#endif
	//the lower time before time out
  ...
  if (lower_tv->tv_usec < 0) {
    lower_tv->tv_usec = lower_tv->tv_usec + 1000000; // 1s = 1000000us
    lower_tv->tv_sec--;
  }
...
int osip_set_message_callback(osip_t *config, int type, osip_message_cb_t cb) {
	// The OSIP_MESS... is a num of enum type
  if (type >= OSIP_MESSAGE_CALLBACK_COUNT) {
